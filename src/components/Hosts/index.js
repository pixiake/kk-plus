...
const Hosts = () => {
    ...
    const  handleEdit = (key) => {
        ...
        if (configuration.spec.roleGroups.registry.includes(key))
        {
            roles.push("镜像仓库节点")
        }
    ...
    const handleDelete = (key) => {
        ...
        let registry = configuration.spec.roleGroups.registry.filter((item) => item !== key)
    ...
    const handleOk = () => {
        ...
        let registry = configuration.spec.roleGroups.registry.filter((item) => (item !== host.name))
